...
namespace SolrNet.Impl {
    ...
    public class SolrConnection : ISolrConnection {
        ...
        //yskwun 10131017 modify
        public SolrConnection(string serverURL) : this(serverURL, "") {
        }

        //++ yskwun 10131017 credentials connection add
        public SolrConnection(string serverURL, string credentials) {
            ServerURL = serverURL;
            Timeout = -1;
            Cache = new NullCache();
            if (!String.IsNullOrEmpty(credentials)) {
                string username = credentials, password = "";
                string[] sp = credentials.Split(':');
                if (sp.Length == 2) {
                    username = sp[0];
                    password = sp[1];
                }
        ...
        /// <returns></returns>
        // yskwun 20131011 private=>public 
        public string GetQuery(IEnumerable<KeyValuePair<string, string>> parameters) {
