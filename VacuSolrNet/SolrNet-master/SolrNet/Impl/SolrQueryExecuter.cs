...
namespace SolrNet.Impl {
    ...
    public class SolrQueryExecuter<T> : ISolrQueryExecuter<T> {
        ...
        public IEnumerable<KeyValuePair<string, string>> GetAllParameters(ISolrQuery Query, QueryOptions options) {
            ...
            if(!string.IsNullOrEmpty(options.WT)) //yskwun 20130325
                yield return KV.Create("wt", options.WT);
        }
        ...
        public SolrQueryResults<T> Execute(ISolrQuery q, QueryOptions options) {
            ...
            string handlerUrl = this.Handler; //yskwun 20130325
            if(!string.IsNullOrEmpty(options.QT))
                handlerUrl = options.QT;

            var r = connection.Get(handlerUrl, param);
